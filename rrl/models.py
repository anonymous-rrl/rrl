...
class RRL:
    ...
    def rule_print(self, feature_name, label_name, train_loader, file=sys.stdout, mean=None, std=None):
        ...
        for label_id in range(self.net.layer_list[-1].output_dim):
            Wl, bl = list(self.net.layer_list[-1].parameters())
            bias = torch.sum(Wl[label_id][always_act_pos]) + bl[label_id]
            print('Class: {}, \tbias = {:.4f}\n'.format(label_name[label_id], bias), file=file)
            ...
            for k, v in kv_list:
                rid = k
                print('weight = {:.4f}'.format(v), file=file)
                print(self.net.layer_list[-1 + rid[0]].rule_name[rid[1]], end='\n\n', file=file)
            print('#' * 60, file=file)
